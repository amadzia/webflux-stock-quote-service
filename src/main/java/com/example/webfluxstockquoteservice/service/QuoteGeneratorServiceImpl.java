...
public class QuoteGeneratorServiceImpl implements QuoteGeneratorService {
    ...
    public Flux<Quote> fetchQuoteStream(Duration period) {
        ...
        //create quotes, itering on each stock starting at index 0
        return Flux.generate(() -> 0,
                ...
                //we zip that Flux with a Flux.interval, to emit them with a specific period
                .zipWith(Flux.interval(period))
                .map(t -> t.getT1())
                //values are generated in batches, so we need to set their timestamp after their creation
                .map(quote -> {
                    quote.setInstant(Instant.now());
                    return quote;
                })
                .log("com.example.webfluxstockquoteservice.service.QuoteGenerator");
    }
